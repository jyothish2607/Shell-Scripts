...
function lvextend ()
{
 ...
 read -p "Enter the size to extend File System ${FS} [ Ex: 2G ]: " SZ
 ssh -q -o ConnectTimeout=30 -o 'StrictHostKeyChecking no' -T "${USER_NAME}"@"${IP}" FSS="${FS}" SIZE="${SZ}" 'bash -s' << 'ENDSSH'
 date=$(date +%Y-%m-%d)
 CHK_FS=$(sudo /bin/df -hT | grep ${FSS}) > /dev/null 2>&1
 CHK_RESULT=$(echo ${?})
 if [[ "${CHK_RESULT}" -eq 0 ]]
    then
      sudo /sbin/lvextend -L +"${SIZE}" "${FSS}" > /dev/null 2>&1
        if [[ "${?}" -eq 0 ]]
           then
             sudo /sbin/resize2fs "${FSS}"  > /dev/null 2>&1
               if [[ "${?}" -eq 0 ]]
                  then
                    echo -ne "\n";echo "Logical volume ${FS} extended succesfully" | tee -a lvm.log_"${date}"
                    echo -ne "\n";echo "New File system Size";echo "===================="
                    echo ; echo "Filesystem              Type      Size  Used Avail Use% Mounted on"
                    echo "------------------------------------------------------------------"
                    sudo /bin/df -hT | grep -A2 "${FSS}" | tee -a lvm.log_"${date}"
                  else
                    echo "Logical volume ${FSS} not extended succesfully" | tee -a lvm.log_"${date}"
               fi
           else
             echo "lvextend not happened successfully." | tee -a lvm.log_"${date}"
             exit 1
        fi
    else
      echo "File System ${FSS} not found on system" | tee -a lvm.log_"${date}"
 fi
ENDSSH
}
...
function vgextend ()
{
 ...
 for i in "${StringValue[@]}"
     do
       PV_CHECK=$(sudo /sbin/pvdisplay ${i} | grep "Allocatable" | xargs | awk -F " " {'print $2'})
         if [[ "${PV_CHECK}" = "NO" ]]
            then
                sudo /sbin/vgdisplay "${VG}" > /dev/null 2>&1
                VGRESULT=$(echo ${?})
                  if [[ "${VGRESULT}" -eq "0" ]]
                      then
                         sudo /sbin/vgextend "${VG}" "${i}" > /dev/null 2>&1
                             if [[ "${?}" -eq "0" ]]
                                then
 ...
 if [[ "${err}" -eq "1" ]]
    then
      ...
      echo "==========================================="
      sudo /sbin/vgdisplay "${VG}" | grep -i "VG Size"
 fi
...
function pvcreate ()
{
  ...
  for i in "${StringValue[@]}"
      do
          sudo /sbin/pvdisplay "${i}" > /dev/null 2>&1
           RESULT1=$(echo ${?})
             if [[ "${RESULT1}" -ne "0" ]]
                then
                  sudo /bin/df -hT | grep -i "${i}" > /dev/null 2>&1
                  RESULT2=$(echo ${?})
                    if [[ "${RESULT2}" -ne "0" ]]
                       then
                          sudo /sbin/pvcreate "${i}" &>> lvm.log_"${date}"
                             if [[ "${?}" -eq "0" ]]
                                then
...
function showdisks()
{
     ...
     SHOW_DISKS=$(sudo /bin/lsblk | sed 1d | grep "disk" | awk -F " " '{print $1}')
     for i in $(echo "${SHOW_DISKS}" | tr ' ' '\n')
      do
          sudo /sbin/pvdisplay /dev/"${i}" > /dev/null 2>&1
           RESULT1=$(echo ${?})
             if [[ "${RESULT1}" -ne "0" ]]
                then
                  sudo /bin/df -hT | grep -i "/dev/${i}" > /dev/null 2>&1
                  RESULT2=$(echo ${?})
                    if [[ "${RESULT2}" -ne "0" ]]
                       then
                          DISK_FOUND=$(sudo /sbin/fdisk -l | grep ${i} | awk -F " " {'print $2 $3 $4'})
                          echo "${DISK_FOUND}"
                       else
